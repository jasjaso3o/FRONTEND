  ...
  return (
    <div className="publicacion-card bg-white p-4 sm:p-6 rounded-xl shadow-lg w-full max-w-xl mx-auto my-4 transition-shadow hover:shadow-xl" onClick={detallePublicacion}>
      <div className="publicacion-header flex justify-between items-center mb-3">
        <div className="perfil-info flex items-center" onClick={verPerfil}>
      ...
      {imagen ? (
        <div className="publicacion-contenido-con-imagen flex gap-4">
          <div className="publicacion-texto flex-1 min-w-0">
            <h2 className="publicacion-titulo text-xl sm:text-2xl font-bold text-gray-900 mb-2">{titulo}</h2>
            <p className="publicacion-descripcion text-gray-700 leading-relaxed">{descripcion}</p>
          </div>
      ...
      <div className="publicacion-acciones flex items-center justify-start border-t border-gray-100 pt-3 mt-3 space-x-6">
        <button className="accion-btn like-btn flex items-center text-gray-500 hover:text-red-500 transition duration-150 group" onClick={handleLike}>
          ...
          <span className="contador text-sm font-medium">{meGusta}</span>
        </button>
        ...
        <button className="accion-btn dislike-btn flex items-center text-gray-500 hover:text-red-500 transition duration-150 group" onClick={handleDislike}>
          ...
          <span className="contador text-sm font-medium">{noMeGusta}</span>
        </button>
        ...
        <button className="accion-btn comentarios-btn flex items-center text-gray-500 hover:text-green-500 transition duration-150 group" onClick={handleCommentView}>
          ...
          <span className="contador text-sm font-medium">{comentarios}</span>
        </button>

        {soyPropietario && (
          <button className="text-red-600 font-semibold hover:text-red-800" onClick={handleDelete}>Eliminar</button>
        )}
      </div>
    </div>
  )
